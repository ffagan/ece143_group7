...
def clean_UCI_s(extra=False):
    '''
    This function extracts the data and performs cleaning for the Switzerland data file
    (switzerland.data) from the UCI dataset
    ...
    z=[]
    for i in range(num):
        if (data[i][12]== -9 or data[i][9]==-9 or data[i][15]==-9) and (i not in z):
            z.append(i)
